...
One of the most common mistakes that new Node.js developers make is assuming that code written after a callback function will be run after the callback is returned. After all, this is how programs are usually execute: One line after the previous one has finished running. However, with callback functions that are executed after I/O work, the Node.js runtime will register those callbacks, then continue executing code while the callback waits for data to be returned. Lets look at a quick example:
...
### **Use `npm audit`, Be Aware of Secutiy Issues**
