...
At its core, Node.js (usually just called _Node_) is a JavaScript runtime that executes your JavaScript code using Google's Chrome V8 engine. In the past, JavaScript was primarily run in the browser (many of which run with Chrome V8 engine as well).
...
You can do nearly everything with Node.js that you are accustomed to doing in C, C++, or RPG. In fact, you can now use the Eradani Connect Node.js Framework to call all of the RPG programs, SQL procedures, CL and QSH commands, and much more that you have invested a lot of time and money in, **allowing Node.js to build on the work that you have already done**. We will talk more about interaction between the Node.js runtime and OS400 more below.

The Node.js runtime is supported by an extremely rich ecosystem of open-source packages which you can use in your applications for free. The **_over one million packages_** include, but are _certainly_ not limited to `request` and `express` to help you easily interact with the internet, `os` to help you interact with your operating system, and `eradani-connect-node` to help you easily interact with native programs on the IBM i.

What does this mean for you as a developer? It means that when you sit down to write an application, **it's not just you.** You have the full backing of millions of developers around the world in the form of a rich library of packages to use in your programs. **A typical Node.js application will have over 10,000 contributing developers!** Imagine how long much time you will save by reusing code that has already been written for things like calling or serving HTTP endpoints. This frees you to focus almost exclusively on your own application logic, greatly increasing development time compared to other technologies.
...
One of the most compelling reasons to use Node.js is npm, The Node Package Manager. npm is a repository of over 1,000,000 open-source Node.js packages have been developed by members of the community, and can be found at [https://www.npmjs.com/](https://www.npmjs.com/). This makes npm the largest code repository in the entire world! To browse the packages available, simply visit the website and search in the bar for either the package name or by keywords.

The packages on npm range in size and scope from single-line functions that do very specific data testing or manipulation, to entire frameworks for creating web servers and creating API endpoints. Many packages are updated very frequently and have tens of millions of downloads every week. The most popular package on npm, `lodash`, is a utility library that has 24 million weekly downloads.
...
### **Node.js Packages For IBM i**

As mentioned above, one of the main benefits of using Node.js is the robust package ecosystem that is available on npm that allows you to implement complex operations out of the box without reinventing the wheel. As part of this ecosystem, several groups such as the IBM i Open-Source Software Team have developed a number of packages that help you interact with your IBM i, either from a Node.js process running directly on your IBM i or a remote process running on a server or your development machine.

Because Open Source on IBM i is an emerging field, these interaction technologies are constantly evolving with new specializations. To name a few: 

* **`idb-connector`**: A database connector that creates a simple API for communicating with Db2 on the IBM i. Effective for database queries, but incapable of calling PGMs without first writing stored procedures in Db2 and can only be used in a Node.js instance running on an IBM i directly - no remote calling capabilities.

* **`idb-pconnector`**: A wrapper for `idb-connector` that converts the API to use native JavaScript `Promises` instead of callback functions, and also comes with connection pooling features. Same limitations as the `idb-connector`.

* **`itoolkit`**: An extremely versatile toolkit which is theoretically capable of initiating any native IBM i operation, albeit at a performance cost. IBM has recently moved away from supporting this toolkit.

* **`odbc`**: The latest database connector, ODBC is platform (and database management system) agnostic and provides a standard interface to your IBM i's Db2. ODBC appears to be IBM's preferred direction today.

* **`eradani-connect`**: We recognize that each of the available connectors is specialized and goes through support cycles with IBM and other third-party providers. We created the Eradani Connect Framework to standardize this interface and help you get the best of all worlds. At the surface level, Eradani Connect provides a uniform interface for you to access any IBM i resource throughout your Node.js programs securely and efficiently. Under the hood, Eradani Connect orchestrates all of the major IBM i connectors so that each program call can always use the best connector for its specific operation. Eradani Connect also automatically translates between Open Source standards and IBM i standards so that all developers can work in comfortable environments, no matter which side they are on.
...
When you download Node.js, in addition to the `node` command you also get the `npm` command. The `npm` utility installs packages from [https://www.npmjs.com/](https://www.npmjs.com/) and manages the metadata for your program (e.g. version number, author name, scripts for running and testing, etc.).
...
`package.json` is the most important file for tracking the metadata of your program. It contains a number of fields that describe your program like the author and version number, declares how to run the program and the test scripts, and most importantly, keeps track of the dependencies for your program. Let's take a look at a sample `package.json`.
...
`package.json` describes your Node.js application, but currently isn't _too_ interesting. To see the real power of this file, let's add some dependencies for our application. Let's say we need a framework for creating APIs (`express`), an XML parser (`xml2js`), and a utility package to help with common programming functions (`lodash`). To be able to download these to use in our program, we are going to use `npm install`:
...
The real power of `package.json` comes when we look at the size of `node_modules`: 6 MB! If we wanted to keep track of the changes to our Node.js in a source control system like `git`, we would have to track this 6MB of files as well, as they are an important part of our program. Luckily, `package.json` makes it so we don't have to track all those files: It gives npm a roadmap to reinstall all the programs we need!
...
Next, we are going to create a JavaScript file, where were import the functionality of `request` and use it to do something interesting. Create a file called `app.js` and include the following code:
...
In the first line of our program, we use the `require` module (a native part of Node.js) to import the object (in this case a function) exposed by the `request` package, and assign it to the variable `request`. There are a number of places where `require` will look to resolve the name given to it, but for now we should only be concerned that it looks in our local `node_modules` folder where `request` was placed when we installed it.

We then use the function to call a free API endpoint, which returns some dummy JSON data.

Note that this request function takes two parameters: A string which is the URL of the online resource we want to get, and a function (which itself has 3 parameters). This function is a **_callback function_** and is an important part of programming in Node.js.

When you do some sort of I/O work or other long-running task, older programming languages would simply wait and do nothing until the operation was completed. Node.js, on the other hand, allows you to specify callback functions which will run _when the long-running task has completed_, thereby allowing other parts of the program to operate while Node.js waits for that task to complete. We specify a callback function to the `request` function so that when the API call returns an answer, the data from the API call (and errors, if any) will be passed to our callback function for processing.
...
The real power of Node.js comes from the million open-source packages hosted on npmjs.com that allow you to use someone else's hard work to implement functionality in your program. This allows you to focus primarily on your business logic and not on reinventing the wheel.
...
If you are interested in developing and running business applications with Node.js, there are important aspects of the runtime and ecosystem that you must keep in mind. Below you will find important information on the runtime and how to avoid common mistakes, as well as some links for further reading.
...
Node.js is very fast. It can handle thousands of requests per second and route them to the correct part of an application stack, handle I/O operations quickly, and is even fairly performant at CPU-bound activities. However, if the backend processes that Node.js is executing are slower than requests are coming into the Node.js process, it may seem like Node.js is the culprit when really it is not. The IBM i is a very performant system that was designed to quickly manipulate large record sets, but this doesn't mean that it can't be the bottleneck for your application. Be sure you are profiling your stack to determine where bottlenecks occur in your system.

---

### **Use `npm audit`, Be Aware of Security Issues**

The biggest fear most people have when using npm is the fact that it installs third-party code on their system that is used by their applications. While most large packages are well-vetted, and you shouldn't be paralyzed with fear when installing packages from npm, it is always a good idea to ocassionally audit the packages you have included with `npm audit`. When you run this command in your Node.js project, it will search through all of the code you have downloaded and alert you if any security vulnerabilites have been reported for any package in your project. **It is also typically best to download packages with large numbers of weekly downloads.** If an npm package is being downloaded 50,000 times per week, you can bet that if there is a security vulnerability, it will be reported and fixed very quickly.
