...
Like the other generators, the `lb model` CLI will asks us a number of questions about our model. For our base class, e use PeristedModel because we want to keep our data in a database. We want to be able to access our model through an API (public, but secured) so we specify `yes` for exposing it. We can specify a plural if we so choose.
