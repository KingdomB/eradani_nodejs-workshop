...
One of the reasons JavaScript is so popular is that it is easy to learn and use. Because it is weakly typed, developers do not have to worry about keeping track of data types, which allows for greatly increased development speed. As we will eventually see in this lab, that can become an issue in larger projects, but for now we will use it to our advantage.
