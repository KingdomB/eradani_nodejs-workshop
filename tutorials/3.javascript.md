...
* `var`: The old way of declaring variables. It has some quirks. *Do not use `var`, unless you really know why you need to use it.*
...
**Note:** the indentation of the code inside the `if` block is just aesthetic to increase readability. Unlike languages like Python or RPG, JavaScript is not whitespace- or position-sensitive.
...
Finally, `if` statements can be extended to use `else if`, which will execute if a condition is `true` _and_ all above `if`s and `else if`s above it are `false`. When you write a chain of `if/else if/else` blocks, your program will check each one starting from the top. Once it finds a condition that evaluates to `true`, the program stops checking. In your editor, enter the following code:
...
* Null: Can only be `null`. Not to be confused with `undefined`, `null` is often used to indicate a value has been set, and that value is nothing.
...
In software development it is usually advised to separate code into discrete chunks of code called _functions_ that can be called by other parts of your program.
...
If you were to print the value of `num` after the function runs, what would you expect it to be? You might say 1 because of the increment in line 2 of the code, but it is actually `0`. When we pass a primitive value (strings, numbers, etc) into a function, we pass it _by value_ (with a few exceptions). That is, the function actually receives an equivalent copy of `num` so the value of `num` cannot be changed from inside the function. Instead, the parameter acts as a new variable inside of the function.

Also remember, there is no type-checking in JavaScript functions. If a string which wasn't a number had been sent to the function, the program would have run just fine until it tried to execute the line `value++`, at which point the value of the variable value is converted to a Number (because thats what the `++` operator expects) and becomes `NaN`, meaning Not-A-Number. If type-checking is important for your programs (and it probably should be), you would either have to explicity check parameter types inside the function, or use something like [TypeScript](https://www.typescriptlang.org/), which we will cover later in the lab.
...
When iterating over an entire array, we can also use a nice shorthand: `for-let-of`
...
Through the examples above, we have walked through the basics of programming with JavaScript. While there are many more keywords to learn, built-in functions for manipulating data, and plenty more gotchas, you have learned enough to develop interesting programs in Node.js! The best way to learn the rest of JavaScript is simply to start programming, and as you encounter issues or desire certain functionality (splitting strings, concatenating arrays, etc.), just Google it! You will quickly become familiar with what is available and become comfortable with JavaScript syntax, making you a formidable Node.js developer.

In the next section, we will look at how you can use your new JavaScript skills to develop applications with Node.js, starting with what exactly Node.js is.
