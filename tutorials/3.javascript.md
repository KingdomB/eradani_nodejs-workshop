...
```
Our simple one-line example illustrates some important parts of JavaScript. First, note that I created a comment after the code with `//`. When you start a comment with `//`, everything that comes after until the end of the line is treated as a comment and is not executed. You can also create multi-line comments using `/*` to begin and `*/` to end.
...
**Note:** the indentation of the code inside the `if` block is just aesthetic. Unlike some languages, JavaScript is not whitespace-sensitive.
...
Finally, `if` statements can be extended to use `else if`, which will execute if a condition is `true` _and_ all above `if`s and `else if`s are `false`. In your editor, enter the following code:
...
We could hold an entire lab on JavaScript functions, but for now it is important to know that you can create and call JavaScript functions as we did above.
