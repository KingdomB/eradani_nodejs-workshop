...
**Note:** Hover over this box to see the code for a one-line Node server: <span class="hovershow">`require('http').createServer((request, response) => response.end("Hello Internet!")).listen();`</span>. This is a fully functional Node.js webserver in just one line of code.