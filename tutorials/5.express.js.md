...
[//]: #TODO: Do we want to have them do POST requests at all? Maybe it's best to just do GETs... If we only do GETs, we can do the whole thing through the browser and won't have to have them set up an HTTP client like Postman. While it's not exactly to standards since GET requests by specification shouldn't have side effects, I'm leaning toward doing all of this through GETs.